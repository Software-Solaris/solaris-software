...
if (-not $inp -or $inp -match '^[Yy]') {
    $inp = (Read-Host "  Raspi key name [raspberry]").Trim()
    $raspiKeyName = if ($inp) { $inp } else { "raspberry" }
    ...
    if ($hasRaspi) {
        ...
        $block = "`nHost raspi`n    HostName 192.168.XX.YYY`n    User username`n    IdentityFile ~/.ssh/$raspiKeyName`n    IdentitiesOnly yes"
        Add-Content -Path $sshConfig -Value $block -Encoding UTF8
        Ok "SSH config updated."
        Warn "Edit $sshConfig - replace 192.168.XX.YYY and 'username' with the actual values."
    }
} else {
